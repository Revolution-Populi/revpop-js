...
class GoogleDriveAdapter extends Adapter {
  /**
   * @param {*} options required options: { auth: oAuth2Client| drive: google.drive, folder: "revpop" }
   * Expect options.drive in browser
   * or option.auth in node.js environment - oAuth2Client - initialized OAuth2 object see:
  ...
  constructor(options = {}) {
    ...
    if (this.isBrowser()) {
      if (options.drive != undefined) {
        ...
        throw new Error("Undefined expecting param - options.drive (initialized google.drive object)");
      }
    } else {
